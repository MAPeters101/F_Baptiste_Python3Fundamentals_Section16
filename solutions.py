...
Now let's figure out what parameters we want to define for our function that
will determine an approximate min or max:
...
resolution: indicates how many times we'll evaluate the function func over the
interval [start, end) - default to 1_000
is_min: if True returns the minimum, otherwise returns the maximum - default
to True
Let's define the function first, and we'll come back to it's implementation
later:

def find_extreme(f, start=-10, end=10, resolution=1_000, is_min=True):
    pass
Our first task will be to calculate the list of values we want to evaluate the
function at:
...
[1.0,
 ...
 5.285714285714286,
 6.0]
Ok, so this is starting to look good. Next we need to evaluate the function at
...
def find_extreme(f, start=-10, end=10, resolution=1_000, is_min=True):
    ...
    if is_min:
        ...
        result = max(f_values)
    return result
The first thing to note is that we are creating these lists
(data and f_values) - that seems uncessary - we could use generator expressions
instead since we will only need to iterate through them once:
...
0.9999996994977832
Another thing too, is that we using a comprehension to apply the function func
to every value in data - this is fine, but we could also just use the map
function:
...
find_extreme(f3, -10, 10, is_min=False)
0.9999996994977832
Additionally, we could use a ternary operator to pick whether we should use
min or max:
...
results
[0.0, 1.4142135623730951, 22.360679774997898, 4.154354402313314]
But whenever we see code that creates an empty list and a loop that just
appends to that list with relatively simple code in the loop body, we should
...
results
[0.0, 1.4142135623730951, 22.360679774997898, 4.154354402313314]
Note: the map function returns a generator, so we pass that to list() to
actually generate a list.

Let's write some functions to encapsulate each technique so we can easily use
them for timing things:
...
log_16(10) --> 0.8304
Solution
For this we'll use a function that defines and returns a nested function, and
captures the function whatever arguments need to be passed to it (aside from
the first argument).
...
f = partial(power, 2)
This function is a closure, and knows about both power and 2. We can see it
this way:
...
args (2,)
kwargs {}
As you can see, the closure captured the power function and the value 2
(in args) - and of course kwargs is empty. If we had a function that requires
keyword only arguments, we could pass those in too:
...
args (10,)
kwargs {'k1': 100}
So, we have the basic skeleton for our solution. What we now need to do is
actually call the function, inserting first_arg, and return the result of that.
...
def partial(f, *args, **kwargs):
    def inner(first_arg):
        ...
        return result
    return inner
We don't actually need to store the result and then return it, we can just
return it directly:
...
find_index_min_logged with some arguments, and not only get the actual
result back, but also see an output to the console that tells you how long the
...
def logged(f):
    pass
Then we'll create a nested function that will receive whatever arguments f
needs to be called, and we'll return that new inner function
(that will be the closure):
...
def logged(f):
    def inner(*args, **kwargs):
        ...
        return result
    return inner
You'll notice that when we call logged(norm) we'll actually get a function
back - that inner function whose f value is actually norm - let's try it out:

logged_norm = logged(norm)
logged_norm
<function __main__.logged.<locals>.inner(*args, **kwargs)>
As expected logged_norm is a function - but it is a special function - it
knows that f (in it's body) is actually norm.

We can actually see it this way:

logged_norm.__closure__
(<cell at 0x7ff281a01a90: function object at 0x7ff2819ece50>,)
Notice how there is this "cell", which is a function object at some memory
address - that memory address is actually the memory address of norm:
...
'0x7ff2819ecee0'
Now let's finish off our logged function - we still need to time things and
print that out:
